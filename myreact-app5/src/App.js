...
function App() {
  ...
  // state는 간단하게 말해서 변수이다 하지만 const, let 등으로 선언한 변수와 달리 값이 변하면 
  // 관련 있는 컴포넌트들이 재렌더링되어 화면이 바뀐다.
  // state 데이터 
  let [jemok,setJemokFunc] = useState('자바스크립트'); // 문자열 기억
  let [jemok2,setJemokFunc2] = useState(['리액트','뷰']); // 배열 자료를 기억
